...
resource "libvirt_domain" "domain_master" {
  for_each  = var.vms
  name      = format("tf-%s_%s_%s", var.project_name, each.key, replace(cidrhost(var.network_cidr, each.value.hostID), ".", "-"))
  ...
  network_interface {
    ...
    addresses      = [cidrhost(var.network_cidr, each.value.hostID)]
    hostname       = format("%s.%s.%s", each.key, var.project_name, var.domain)
  }
